# References

Every entry below was verified against its primary source (publisher page,
arXiv abstract, or official repo) on 2026-08-30; details are copied from
those pages, not from memory. In-text citations in STORY.md / README.md
refer to this list.

**Synesthesia and cross-modal cognition**

Galton, F. (1880). Visualised numerals. *Nature*, 21, 252–256.
https://doi.org/10.1038/021252a0

Seron, X., Pesenti, M., Noel, M. P., Deloche, J., & Cornet, J. A. (1992).
Images of numbers, or "When 98 is upper left and 6 sky blue". *Cognition*,
44(1–2), 159–196. https://doi.org/10.1016/0010-0277(92)90051-K

Ramachandran, V. S., & Hubbard, E. M. (2001). Psychophysical
investigations into the neural basis of synaesthesia. *Proceedings of the
Royal Society B: Biological Sciences*, 268(1470), 979–983.
https://doi.org/10.1098/rspb.2001.1576

Eagleman, D. M., Kagan, A. D., Nelson, S. S., Sagaram, D., & Sarma, A. K.
(2007). A standardized test battery for the study of synesthesia.
*Journal of Neuroscience Methods*, 159(1), 139–145.
https://doi.org/10.1016/j.jneumeth.2006.07.012

Thaler, L., Arnott, S. R., & Goodale, M. A. (2011). Neural correlates of
natural human echolocation in early and late blind echolocation experts.
*PLoS ONE*, 6(5), e20162. https://doi.org/10.1371/journal.pone.0020162

Parise, C. V., & Spence, C. (2012). Audiovisual crossmodal
correspondences and sound symbolism: A study using the implicit
association test. *Experimental Brain Research*, 220(3), 317–329.
https://doi.org/10.1007/s00221-012-3140-6
(page range from the Springer/PNAS citation records; verify against the
article PDF if quoting precisely)

Palmer, S. E., Schloss, K. B., Xu, Z., & Prado-León, L. R. (2013).
Music–color associations are mediated by emotion. *Proceedings of the
National Academy of Sciences*, 110(22), 8836–8841.
https://doi.org/10.1073/pnas.1212562110

Spence, C. (2011). Crossmodal correspondences: A tutorial review.
*Attention, Perception, & Psychophysics*, 73(4), 971–995.
https://doi.org/10.3758/s13414-010-0073-7

**Computational synesthesia and multimodal learning**

Bock, J. R. (2018). A deep learning model of perception in color-letter
synesthesia. *Big Data and Cognitive Computing*, 2(1), 8.
https://doi.org/10.3390/bdcc2010008

Elhage, N., Nanda, N., Olsson, C., Josephson, T., Linsson, R., Henighan,
A., … Olah, C. (2022). Toy models of superposition. *Transformer Circuits
Thread*. https://transformer-circuits.pub/2022/toy_model/index.html

Wienand, D., & Heckl, W. (2021). The artificial synesthete: Image-melody
translations with variational autoencoders. *arXiv preprint*
arXiv:2112.02953. https://doi.org/10.48550/arXiv.2112.02953

Bardes, A., Ponce, J., & LeCun, Y. (2022). VICReg:
Variance-invariance-covariance regularization for self-supervised
learning. *International Conference on Learning Representations (ICLR
2022)*. arXiv:2105.04906. https://doi.org/10.48550/arXiv.2105.04906

Wu, Y., Chen, K., Zhang, T., Hui, Y., Nezhurina, M., Berg-Kirkpatrick, T.,
& Dubnov, S. (2023). Large-scale contrastive language-audio pretraining
with feature fusion and keyword-to-caption augmentation. *IEEE ICASSP
2023*. arXiv:2211.06687. https://doi.org/10.48550/arXiv.2211.06687

SyneState: Inducing machine synesthesia via manifold-constrained
hyper-connections (2026). Zenodo preprint.
https://zenodo.org/records/18180051

**Representation and capability**

Lample, G., & Charton, F. (2020). Deep learning for symbolic mathematics.
*International Conference on Learning Representations (ICLR 2020)*.
arXiv:1912.01412. https://doi.org/10.48550/arXiv.1912.01412

McLeish, S., Bansal, M., Stein, A., Jain, N., Kirchenbauer, J., Bartoldson,
J. R., Kailkhura, B., Bhatele, A., Geiping, J., Yiu-Neumann, S., &
Goldstein, T. (2024). Transformers can do arithmetic with the right
embeddings. *arXiv preprint* arXiv:2405.17399.
https://doi.org/10.48550/arXiv.2405.17399

Kim, C. D., Kim, B., Lee, H., & Kim, G. (2019). AudioCaps: Generating
captions for audios in the wild. *Proceedings of NAACL-HLT 2019*.
https://aclanthology.org/N19-1011/

Witthoft, N., & Winawer, J. (2013). Learning, memory, and synesthesia.
*Psychological Science*, 24(11), 1858–1865.
https://doi.org/10.1177/0956797613479524

---

## Verification notes (2026-08-30)

Entries marked with confidence notes, in the spirit of full disclosure:

- All DOIs and author lists above were read off the live publisher/arXiv
  pages during bibliography construction, not written from memory.
- Two entries carry residual uncertainty, flagged here so a reader can
  check them:
  - **Witthoft & Winawer (2013)**: DOI and journal/volume taken from a
    secondary citation trail (the MDPI paper's reference list and
    ResearchGate); the primary APA page was not fetched. Verify before
    quoting in a camera-ready manuscript.
  - **Wienand & Heckl (2021)**: authors and year confirmed on the arXiv
    abstract page; the figshare supplement (media files) was not checked.
- The Abacus-embeddings paper's full author list is long; "et al."
    truncation follows the arXiv listing order (McLeish et al., 2024).
- Known corrections made during verification (the values previously in
  STORY.md were wrong and were fixed rather than propagated):
  - Eagleman battery: 2007 (Epub 2006), not 2006; co-authors Kagan,
    Nelson, Sagaram & Sarma (per PubMed PMID 16919755).
  - Thaler et al.: Arnott (not "Arnold"), and no Kish.
  - SyneState: a 2026 Zenodo preprint, not a 2025 paper.
  - Wienand & Heckl: two authors only (per arXiv API), not "et al."
  - Lample & Charton: ICLR 2020 (the arXiv preprint is dated 2019; the
    conference version is 2020 — STORY.md's citation was correct).
- Elhage et al. (2022) has no DOI; cited by URL, as is standard for the
  Transformer Circuits Thread.