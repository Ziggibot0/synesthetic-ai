...
All experiments use pre-registered gates, param-matched controls, and deterministic seeds (seed 1337). Code, logs, and result JSONs are at \texttt{github.com/Ziggibot0/synesthetic-ai}.
...
All code, pre-registered designs, per-epoch logs, and result JSONs are at \texttt{github.com/Ziggibot0/synesthetic-ai}. Experiments are deterministic (seed 1337) and reproducible with:
\begin{itemize}
\item \texttt{py -3.12 v0\_semantic\_gate.py} (V0, ${\sim}5$ min CPU)
\item \texttt{py -3.12 v1\_learned\_encoder.py -{}-epochs 200} (V1, ${\sim}20$ min CPU)
\item \texttt{py -3.12 c1\_run\_all.py -{}-epochs 60} (C1, ${\sim}2$h iGPU)
\end{itemize}
